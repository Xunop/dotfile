    ...
    for node in nodes:
        ...
        if not have_overlap:
            ...
            if node.x + newwindow_size.w > canvas_size.w and node.y + newwindow_size.h > canvas_size.h:
                area_in_canvas.append(
                    (canvas_size.w - node.x) * (canvas_size.h - node.y))
                spare_space_size.append(None)
                continue
            if node.x + newwindow_size.w > canvas_size.w:
                area_in_canvas.append(
                    (canvas_size.w - node.x) * newwindow_size.h)
                spare_space_size.append(None)
                continue
            if node.y + newwindow_size.h > canvas_size.h:
                area_in_canvas.append(
                    (canvas_size.h - node.y) * newwindow_size.w)
                spare_space_size.append(None)
                continue
...
def arrange_windows(windows):
    if len(windows) == 0:
        return
    if len(windows) == 1:
        center_pos = Position(canvas_pos.x + canvas_size.w / 2 -
                              windows[0].size.w / 2, canvas_pos.y + canvas_size.h / 2 - windows[0].size.h / 2)
    ...
    for window in windows:
        if window.address == windows[0].address:
            continue
        spare_space_pos, spare_space_size, area_in_canvas = find_spare_space(
            arranged_windows, window.size)
        best_pos = choose_best_spare_space(
            spare_space_pos, spare_space_size, area_in_canvas)
        window.pos = best_pos
    ...
    for window in arranged_windows:
        move_window_to_pos(window.address, Position(
            window.pos.x + dx, window.pos.y + dy))
...
def move_window_to_pos(windowaddress, pos):
    ...
    if x < 0 and y < 0:
        os.system(
            f'hyprctl dispatch movewindowpixel exact \'0 {y}\',address:{windowaddress}')
        os.system(
            f'hyprctl dispatch movewindowpixel {x} 0,address:{windowaddress}')
        return
    if x < 0:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Arrange windows in a smart way', allow_abbrev=False)
    parser.add_argument('workspaces', type=str, nargs='*',
                        help='workspaces to be tiled')
    parser.add_argument('--gap', type=int, nargs='?',
                        help='outer gap of a window', default=5)
    parser.add_argument('--border', type=int, nargs='?',
                        help='border size of a window', default=4)
    parser.add_argument('--canvas_pos', type=str, nargs='?',
                        help='position of the canvas, format: x,y')
    parser.add_argument('--canvas_size', type=str, nargs='?',
                        help='size of the canvas, format: w,h')
    parser.add_argument('--prior', type=str, nargs='*',
                        help='specify windows that should be priorly tiled')
    ...
    if args.canvas_pos == None:
        ...
        canvas_pos = Position(
            *map(int, args.canvas_pos.strip('\'').split(',')))
    ...
    for workspace_id in workspace_ids:
        clients_in_workspace = list(
            filter(lambda client: client['workspace']['id'] == int(workspace_id), clients))

        windows = []
        for client in clients_in_workspace:
            windows.append(Window(client['address'], Position(client['at'][0] - gap - border, client['at'][1] -
                           gap - border), Size(client['size'][0] + 2 * (gap + border), client['size'][1] + 2 * (gap + border))))
